...
function ColorSection() {
  ...
  // Linen & Slate — the only palette
  const swatches = [
    { token: '--sr-color-bg',      label: 'Linen',   hex: '#F4F2EE' },
    { token: '--sr-color-brand',   label: 'Slate',   hex: '#3B4A63' },
    { token: '--sr-color-accent',  label: 'Amber',   hex: '#C68A3B' },
    { token: '--sr-color-success', label: 'Success', hex: '' },
    { token: '--sr-color-warning', label: 'Warning', hex: '' },
    { token: '--sr-color-danger',  label: 'Danger',  hex: '' },
  ];

  return (
    <>
      <div data-palette="linen" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontFamily: 'var(--sr-font-display)', fontWeight: 600, fontSize: 22, letterSpacing: '-0.01em' }}>Linen &amp; Slate</div>
            <div style={{ fontSize: 13, color: 'var(--sr-color-text-subtle)', marginTop: 4 }}>The brand palette · premium, dependable</div>
          </div>
          <code style={{ fontSize: 11, color: 'var(--sr-color-text-subtle)' }}>linen</code>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
          {swatches.map(s => (
            <div key={s.token} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{
                height: 88, background: `var(${s.token})`,
                borderRadius: 'var(--sr-radius-md)',
                border: '1px solid var(--sr-color-border-subtle)',
              }} />
              <div style={{ fontSize: 12, fontWeight: 600 }}>{s.label}</div>
              {s.hex && <code style={{ fontSize: 11, color: 'var(--sr-color-text-subtle)' }}>{s.hex}</code>}
            </div>
          ))}
        </div>
        <div style={{ fontSize: 14, color: 'var(--sr-color-text-muted)', lineHeight: 1.55, maxWidth: '60ch' }}>
          Slate-blue reads as serious and reliable — the kind of brand a busy operator trusts on first call. Amber accent keeps it warm so CTAs feel inviting, not corporate.
        </div>
      </div>

      <div className="sr-subsection-head">Semantic roles</div>
